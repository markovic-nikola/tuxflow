//! Release check, in-place .deb install and relaunch — shared by both
//! shells. Every network call takes the RUNNING version explicitly: this
//! crate's own `CARGO_PKG_VERSION` is not the app's, and comparing against
//! it would light the chip on every launch.
//!
//! Blocking throughout: call from a worker thread.
...
/// worker thread. Returns `None` when `current` is already up to date.
...
/// Download the .deb to a temp path. Blocking. `current` only names the
/// running version in the User-Agent.
pub fn download_deb(current: &str, url: &str) -> Result<PathBuf, String> {
    let config = ureq::Agent::config_builder()
...
///
/// The GTK app is single-instance (GtkApplication holds a D-Bus name), so a
